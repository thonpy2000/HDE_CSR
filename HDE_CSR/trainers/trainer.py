...
def train_parser():
    ...
    parser.add_argument("--epoch", help="number of epochs before lr is cut by gamma", type=int, default=250)
    ...
    parser.add_argument("--train_query_shot", help="number of query images per class during meta-training", type=int, default=7) # 15
    ...
    parser.add_argument("--dataset", choices=['stanford_dog', 'stanford_car', 'cub_raw', 'aircraft','cub_cropped','meta_iNat', 'tiered_meta_iNat'])
    parser.add_argument("--alpha", type=float, default=0.5)
    parser.add_argument("--beta", type=float, default=0.5)
    parser.add_argument("--model", choices=['HDE_CSR'], default='HDE_CSR')
...
def get_opt(model, args):
    ...
    if args.decay_epoch is not None:
        ...
        scheduler = optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=[70, 100], gamma=args.gamma)
    ...
    def evaluate(self, model):
        ...
        with torch.no_grad():
            ...
            try:
                ...
                model.module.load_state_dict(torch.load(self.save_path,  weights_only=True))
            model.eval()
            model.cuda()
